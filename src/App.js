...
function Nav(props){
  ...
  for(let i=0; i<props.topic.length; i++)
  {
    ...
    lis.push(<li key={t.id}><a id={t.id} href={'/read/'+t.id} onClick={(event)=>{
      ...
      // event.target.id가 넘어가면서 문자열로 넘어가기 떄문에 숫자로 바꾸어 넘겨주어야함
    }}>{t.title}</a></li>)
    // event.target은 이벤트를 유발한 태그를 의미하므로 여기선 a 태그를 가르킴
  }
...
function App() {
  ...
  let content = null;
  let contextControl = null; // READ모드 일때만 update가 뜨도록 하기 위한 변수
  if(mode === 'WELCOME'){
  ...
  return (
    <div>
      ...
      <Nav topic={topics} OnChangeMode={(_id)=>{setMode('READ'); setId(_id);}}></Nav>
      {content}
      <ul>
